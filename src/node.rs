...
pub struct Node<'nodes, T, U: Copy> {
    pub(crate) inner: NodeValue<'nodes, T, U>,
}

impl<T, U: Copy> Debug for Node<'_, T, U> {
...
impl<'nodes, T, U: Copy> Node<'nodes, T, U> {
    ...
    pub fn min_width(&mut self, available_area: Area, state: &mut T, u: U) -> Option<f32> {
        self.inner
