...
mod tests {
    ...
    fn test_partial_scope() {
        ...
        Layout::new(dynamic(|_: &mut State| {
            scope_owned::<State, SubState>(
                |state: &mut State| (state.0, &mut state.1.c),
                |a: &mut State, b: SubState| {},
                |_: &mut SubState| {
                    draw(|area, _state: &mut SubState| {
                        assert_eq!(area, Area::new(0., 0., 100., 100.));
                    })
        ...
        .draw(Area::new(0., 0., 100., 100.), &mut state);
    }
